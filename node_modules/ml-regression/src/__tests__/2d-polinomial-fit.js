...
describe('2D polinomial fit', () => {
    ...
    it('Training coefficients', () => {
        const estimatedCoefficients = [1.5587e1, 3.8873e-1, 5.2582e-3, 4.8498e-1, 2.1127e-3, -7.3709e-3];
        for (let i = 0; i < estimatedCoefficients.length; ++i) {
            expect(pf.coefficients[i][0]).toBeCloseTo(estimatedCoefficients[i], 1e-2);
        }
    });
