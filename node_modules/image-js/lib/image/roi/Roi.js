...
class Roi {
  ...
  get contourMask() {
    if (!this.computed.contourMask) {
      let img = new _Image2.default(this.width, this.height, {
  ...
  get centerMask() {
    if (!this.computed.centerMask) {
      let img = new _Shape2.default({ kind: 'smallCross' }).getMask();
  ...
  get hullMask() {
    if (!this.computed.hullMask) {
      const img = new _Image2.default(this.width, this.height, {
        kind: KindNames.BINARY,
        position: [this.minX, this.minY],
        parent: this.map.parent
      });
      ...
      for (let x = 0; x < this.width; x++) {
        for (let y = 0; y < this.height; y++) {
          if ((0, _robustPointInPolygon2.default)(hull, [x, y]) !== 1) {
            img.setBitXY(x, y);
          }
        }
      }
